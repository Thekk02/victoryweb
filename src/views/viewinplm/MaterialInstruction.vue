...
import {getFileByname, getMaterialInstruction} from "@/apis/api";
import GlobalVariable from "@/module/GlobalVariables";
import { ElMessageBox } from 'element-plus';

export default ({
  name:"MaterialInstructionPage",
  computed:{},
  setup(){},
  data(){
    return {
  ...
  methods:{
    inqueryhandler(){
      if(this.MaterialInstruction.name.length < 5){
        ElMessageBox.alert('输入原材料标准名称太短，请重新输入！！！', '查询条件错误', {

        })
      }else{
        getMaterialInstruction(this.MaterialInstruction.name).then((resp) =>{
        ...
        <el-container>
          <el-main class="app-main">
            <div class="app-queryparameters">
              <el-row>
                <el-form-item label="原材料标准名称">
                  <el-col :span="24">
                    <el-input v-model="MaterialInstruction.name" placeholder="请输入原材料标准名称" @keyup.enter="inqueryhandler"></el-input>
                  </el-col>
                </el-form-item>
                <el-form-item>
            ...
            <div class="app-queryresult">
              <el-table :data = "list" empty-text="暂无数据" border height="100%" id="inqueryresults"
                        ...
                        style="width: 100%">
                <el-table-column prop="code" label="原材料标准代号" style="width: 25%"   >
                </el-table-column>
                <el-table-column prop="name" label="原材料标准名称"  style="width: 25%">
                </el-table-column>
                <el-table-column prop="plocation" label="原材料标准地址" style="width: 25%">
                </el-table-column>
